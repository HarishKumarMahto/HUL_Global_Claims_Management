    ...
    <div className="flex flex-col w-full h-full">
      <input
        ...
        {claim.substantiationDocs.length === 0 ? (
          <div className="text-xs text-gray-400 italic p-4 bg-white border-t border-pebble flex-1">No documents uploaded. Click "Add Document" to attach substantiation evidence.</div>
        ) : (
...
export default function ClaimsTable({
  ...
  const prevWorkViewRef = React.useRef(activeWorkView);
  // US-M4-015 to 030: Auto-expand rows and sync section when a Work View is selected from Sidebar
  useEffect(() => {
    if (prevWorkViewRef.current === activeWorkView) return;
    prevWorkViewRef.current = activeWorkView;
                          ...
                          <div className="bg-white rounded-lg border border-pebble overflow-hidden shadow-sm">
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <span className="text-[10px] text-gray-400 border-r border-pebble pr-3">({claim.substantiationDocs.length} docs)</span>
                                  ...
                                  {!!onClaimsChange && (
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.preventDefault();
                                        e.stopPropagation();
                                        if (!isSectionOpen(claim.uniqueRowId, 'support')) {
                                          toggleSection(claim.uniqueRowId, 'support');
                                        }
                                        fileRefs.current[claim.id]?.click();
                                      }}
                            ...
                            {isSectionOpen(claim.uniqueRowId, 'risk') && (
                              <div className="p-0 border-t border-pebble bg-pale/5 flex flex-col" style={{ height: '160px' }}>
                                ...
                                {claim.riskAssessments.length === 0 ? (
                                  <p className="text-[10px] text-gray-400 italic p-4 bg-white border-t border-pebble flex-1">No risk assessments recorded.</p>
                                ) : (
                                  <div className="overflow-auto bg-white border-t border-pebble flex-1 relative">
                                    <table className="w-full text-xs">
                                      <thead className="sticky top-0 bg-earth/60 z-10">
