...
export default function CancelDocumentModal({
  isOpen, document, onClose, onConfirm, allDocuments = [],
  ...
  // Block Formulation Documents with linked products
  const isBlocked = document.documentType === 'Formulation Document' && linkedProductCount > 0;
  ...
  const handleConfirm = () => {
    ...
    const updated: DocumentRecord = {
      ...
      linkedAssetIds: [],
      linkedProductIds: [],
      linkedProjectIds: document.documentType === 'Project Document' ? [] : document.linkedProjectIds,
    };
    onConfirm(updated);
  };
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-pebble flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-night">Cancel Document</h2>
              <p className="text-xs text-gray-400 mt-0.5 truncate max-w-[280px]">{document.name}</p>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-xl">
              ...
              <div>
                ...
                <p className="text-xs text-red-600 mt-1">
                  This Formulation Document is currently linked to <strong>{linkedProductCount}</strong> product{linkedProductCount !== 1 ? 's' : ''}.
                  Remove all product linkages before cancelling.
                </p>
              </div>
            </div>
          )}
            ...
            <>
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-2">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs font-bold text-amber-700 uppercase tracking-wide">Impact Summary</span>
                </div>
                {linkedClaimCount > 0 && (
                  <p className="text-xs text-amber-700">
                    • <strong>{linkedClaimCount}</strong> linked Claim{linkedClaimCount !== 1 ? 's' : ''} will be unlinked from this document
                  </p>
                )}
                {linkedAssetCount > 0 && (
                  <p className="text-xs text-amber-700">
                    • <strong>{linkedAssetCount}</strong> linked Asset{linkedAssetCount !== 1 ? 's' : ''} will be unlinked from this document
                  </p>
                )}
                {linkedProjectCount > 0 && document.documentType === 'Project Document' && (
                  <p className="text-xs text-amber-700">
                    • <strong>{linkedProjectCount}</strong> Project linkage{linkedProjectCount !== 1 ? 's' : ''} will be removed
                  </p>
                )}
                {linkedClaimCount === 0 && linkedAssetCount === 0 && linkedProjectCount === 0 && (
                  <p className="text-xs text-amber-600">No active linkages — safe to cancel.</p>
                )}
                <p className="text-xs text-amber-700 mt-2 font-medium">
                  This action cannot be undone. The document will move to <strong>Cancelled</strong> state.
                </p>
              </div>
