...
export default function CloneProjectModal({
  ...
  const [projectName, setProjectName] = useState(
    `${sourceProject.name} (Clone)`,
  );
  const [businessGroup, setBusinessGroup] = useState(
    sourceProject.businessGroup || BUSINESS_GROUPS[0],
  );
  const [category, setCategory] = useState(sourceProject.category || "");
  const [projectType, setProjectType] = useState(
    sourceProject.type || PROJECT_TYPES[0],
  );
  const [startDate, setStartDate] = useState(
    sourceProject.startDate || new Date().toISOString().slice(0, 10),
  );
  const [evaluationDate, setEvaluationDate] = useState(
    sourceProject.evaluationDate || "",
  );
  ...
  const mockClaimsList = [
    {
      id: "c1",
      label: "Claim #1: 100% Superior Stain Removal",
      status: "Active",
    },
    {
      id: "c2",
      label: "Claim #2: Dermatologist Recommended",
      status: "Active",
    },
  ...
  const mockTeamList = [
    {
      id: "t1",
      label: `${sourceProject.projectLead || "Sarah Johnson"} (Project Lead)`,
      status: "Active",
    },
    {
      id: "t2",
      label: `${sourceProject.claimsLead || "Michael Chen"} (Claims Lead)`,
      status: "Active",
    },
    { id: "t3", label: "Dr. Emily Vance (R&D Expert)", status: "Active" },
  ];

  const mockGeoList = (sourceProject.region || "North America, Europe")
    .split(",")
    .map((g) => g.trim())
    .map((geo, idx) => ({
      id: `g${idx}`,
  ...
  const [selectedClaims, setSelectedClaims] = useState<string[]>(
    mockClaimsList.map((c) => c.id),
  );
  const [selectedProducts, setSelectedProducts] = useState<string[]>(
    mockProductsList.map((p) => p.id),
  );
  const [selectedAssets, setSelectedAssets] = useState<string[]>(
    mockAssetsList.map((a) => a.id),
  );
  const [selectedTeam, setSelectedTeam] = useState<string[]>(
    mockTeamList.map((t) => t.id),
  );
  const [selectedGeo, setSelectedGeo] = useState<string[]>(
    mockGeoList.map((g) => g.id),
  );
  const [selectedDocs, setSelectedDocs] = useState<string[]>(
    mockDocList.map((d) => d.id),
  );

  // Step 2 state
  const [subscribeToSourceChanges, setSubscribeToSourceChanges] =
    useState(true);
  ...
  const handleNext = () => {
    ...
    if (!projectName.trim()) {
      ...
      errors.push(
        "Project Name must be unique. A project with this name already exists.",
      );
    }
    ...
    if (
      evaluationDate &&
      launchDate &&
      new Date(launchDate) < new Date(evaluationDate)
    ) {
      errors.push(
        "Target Launch Date cannot be earlier than Target Evaluation Date.",
      );
    }
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="bg-earth border-b border-pebble px-6 py-3 flex items-center gap-6 text-xs font-semibold flex-shrink-0">
          <div
            className={`flex items-center gap-2 ${step === "select" ? "text-sky font-bold" : "text-gray-500"}`}
          >
            <span
              className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${step === "select" ? "bg-sky text-white" : "bg-pebble text-gray-600"}`}
            >
              1
            </span>
          ...
          <div
            className={`flex items-center gap-2 ${step === "notify" ? "text-sky font-bold" : "text-gray-500"}`}
          >
            <span
              className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${step === "notify" ? "bg-sky text-white" : "bg-pebble text-gray-600"}`}
            >
              2
            </span>
            ...
            <>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
                ...
                <div className="text-xs text-blue-800 leading-relaxed">
                  ...
                  <p>
                    Only the BG BU Lead, Business Admin, Project Lead, or Claims
                    Lead of the source project's BG can clone. By proceeding,
                    you verify your authorization.
                  </p>
                  {sourceProject.confidentialRestrictions && (
                    <p className="mt-2 text-purple-700 font-semibold flex items-center gap-1">
                      ...
                      restrictions apply: restricted fields/records are excluded
                      unless permitted.
                    </p>
                  )}
                </div>
              ...
              <div className="bg-earth p-5 rounded-xl border border-pebble space-y-4">
                <h3 className="text-sm font-bold text-night flex items-center gap-2 border-b border-pebble pb-2">
                  <Layers className="w-4 h-4 text-sky" /> Cloned Project
                  Identity & Metadata
                </h3>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs text-gray-600 font-semibold mb-1 uppercase">
                      Project Name *
                    </label>
                    <input
                      type="text"
                    ...
                    <p className="text-[10px] text-gray-500 mt-1">
                      Must be unique across all active projects.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-500 mt-1">
                      If source BG is obsolete, pick active BG.
                    </p>
                  </div>
              ...
              <div className="space-y-3">
                <h3 className="text-sm font-bold text-night flex items-center justify-between border-b border-pebble pb-2">
                  ...
                  <span className="text-xs text-gray-500 font-normal">
                    Inactive/Expired records show disclaimer but can still be
                    copied
                  </span>
                </h3>
                ...
                {[
                  {
                    ...
                    total: mockClaimsList.length,
                    list: mockClaimsList,
                    selected: selectedClaims,
                    setSelected: setSelectedClaims,
                  },
                  {
                    id: "products",
                  ...
                  },
                  {
                    ...
                    count: selectedGeo.length,
                    total: mockGeoList.length,
                    list: mockGeoList,
                  ...
                  const allSelected =
                    section.selected.length === section.list.length &&
                    section.list.length > 0;

                  return (
                    <div
                      key={section.id}
                      className="border border-pebble rounded-xl overflow-hidden bg-white shadow-sm transition-all"
                    >
                      {/* Section Bar */}
                      <div className="px-4 py-3 bg-earth/50 flex items-center justify-between gap-4">
                        ...
                        >
                          {isExpanded ? (
                            <>
                              Hide Records <ChevronUp className="w-3.5 h-3.5" />
                            </>
                          ) : (
                            <>
                        ...
                        <div className="p-4 bg-white border-t border-pebble space-y-3 animate-fade-in">
                          <div className="flex items-center justify-between border-b border-gray-100 pb-2">
                            ...
                            <span className="text-[10px] text-gray-400 font-medium italic">
                              Check individual items below to include in clone
                            </span>
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            {section.list.map((item) => {
                              ...
                              return (
                                ...
                                >
                                  <div
                                    className={`w-4 h-4 rounded border mt-0.5 flex items-center justify-center flex-shrink-0 ${isSel ? "bg-sky border-sky text-white" : "border-gray-300"}`}
                                  >
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    {isInactive && (
                                      <span
                                        className={`inline-block mt-1 px-2 py-0.5 rounded text-[10px] font-bold ${item.status === "Expired" ? "bg-amber-100 text-amber-800" : "bg-red-100 text-red-800"}`}
                                      >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-purple-900">
                      Subscribe to Source Project Notifications
                    </h3>
                    <p className="text-xs text-purple-700 mt-0.5">
                      Stay synchronized with upstream master record updates
                    </p>
                  </div>
                </div>

                <p className="text-xs text-purple-800 leading-relaxed font-medium">
                  When changes occur in the source project's records (e.g.,
                  claims modified, substantiation updated, or scope adjusted),
                  project team members will receive automated notification
                  alerts. Users can then manually review and import these
                  changes into the cloned project.
                </p>

                <div
                  className="bg-white p-4 rounded-xl border border-purple-200 flex items-start gap-3 cursor-pointer"
                  onClick={() =>
                    setSubscribeToSourceChanges(!subscribeToSourceChanges)
                  }
                >
                  <div
                    className={`w-5 h-5 rounded-lg border mt-0.5 flex items-center justify-center transition-colors ${subscribeToSourceChanges ? "bg-purple-600 border-purple-600 text-white" : "border-gray-300 bg-white"}`}
                  >
                  ...
                  <div>
                    <p className="text-sm font-bold text-night">
                      Subscribe to change notifications for source project
                      records
                    </p>
                    <p className="text-xs text-gray-500 mt-0.5">
                      Highly recommended to maintain compliance consistency with
                      the parent project.
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-earth p-5 rounded-xl border border-pebble flex items-center justify-between text-xs">
                <div>
                  ...
                  <p className="text-gray-500 mt-1">
                    A self-referential link "Copied From" pointing to{" "}
                    {sourceProject.name} will be stored.
                  </p>
                </div>
