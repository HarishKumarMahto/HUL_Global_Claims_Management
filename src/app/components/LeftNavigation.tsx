...
import {
  ChevronDown, ChevronRight, ChevronLeft, Star, Clock, FolderOpen,
  Bookmark, LayoutGrid, Info, Users, Globe, Package,
  FileText, Paperclip, Layers, GitBranch, Shield, History, Plus
} from 'lucide-react';
...
const PROJECT_VIEWS = [
  { id: 'My Projects', label: 'My Projects', icon: <FolderOpen className="w-4 h-4" /> },
  { id: 'All Projects', label: 'All Projects', icon: <LayoutGrid className="w-4 h-4" /> },
  { id: 'Recent Projects', label: 'Recent', icon: <Clock className="w-4 h-4" /> },
  { id: 'Favorites', label: 'Favorites', icon: <Star className="w-4 h-4" /> },
  { id: 'Saved Views', label: 'Saved Views', icon: <Bookmark className="w-4 h-4" /> },
];

const ASSET_VIEWS = [
  { id: 'My Assets', label: 'My Assets', icon: <FolderOpen className="w-4 h-4" /> },
...
const ASSET_WORKSPACE_SECTIONS = [
  { id: 'Asset Details', label: 'Asset Details', icon: <Info className="w-4 h-4" /> },
...
const PRODUCT_VIEWS = [
  { id: 'My Products', label: 'My Products', icon: <FolderOpen className="w-4 h-4" /> },
  { id: 'All Products', label: 'All Products', icon: <LayoutGrid className="w-4 h-4" /> },
  { id: 'Recent Products', label: 'Recent', icon: <Clock className="w-4 h-4" /> },
  { id: 'Favorite Products', label: 'Favorites', icon: <Star className="w-4 h-4" /> },
  { id: 'Saved Views', label: 'Saved Views', icon: <Bookmark className="w-4 h-4" /> },
  { id: 'Product Hierarchy', label: 'Product Hierarchy', icon: <GitBranch className="w-4 h-4" /> },
];

const WORKSPACE_SECTIONS: { id: WorkspaceSection; label: string; icon: ReactElement }[] = [
  { id: 'Project Details', label: 'Project Details', icon: <Info className="w-4 h-4" /> },
  { id: 'Geography', label: 'Geography', icon: <Globe className="w-4 h-4" /> },
  { id: 'Project Team', label: 'Project Team', icon: <Users className="w-4 h-4" /> },
  { id: 'Linked Products', label: 'Linked Products', icon: <Package className="w-4 h-4" /> },
  { id: 'Related Claims', label: 'Related Claims', icon: <FileText className="w-4 h-4" /> },
  { id: 'Linked Assets', label: 'Linked Assets', icon: <Paperclip className="w-4 h-4" /> },
  { id: 'Project Documents', label: 'Project Documents', icon: <FileText className="w-4 h-4" /> },
...
export default function LeftNavigation({
  ...
  const [isSavedViewsFolderExpanded, setIsSavedViewsFolderExpanded] = useState(true);
  const [isProductSavedViewsFolderExpanded, setIsProductSavedViewsFolderExpanded] = useState(true);
  const [isRelatedClaimsExpanded, setIsRelatedClaimsExpanded] = useState(true);
  const [expandedClaimSections, setExpandedClaimSections] = useState<Record<ClaimBaseView, boolean>>({
    'Global Claims': true,
  ...
  const [claimSubOpen, setClaimSubOpen] = useState<Record<string, 'recents' | 'favorites' | null>>({});
  const [isCollapsed, setIsCollapsed] = useState(false);

  const toggleSidebar = () => setIsCollapsed(!isCollapsed);

  const sidebarClass = `bg-white border-r border-pebble flex flex-col flex-shrink-0 transition-all duration-300 ease-in-out relative ${isCollapsed ? 'w-0' : 'w-64'
    }`;
        ...
        <div className={`flex flex-col h-full w-64 ${isCollapsed ? 'invisible opacity-0' : 'visible opacity-100'} transition-all duration-200`}>
                ...
                return (
                  <div key={section.id} className="space-y-0.5">
                    <div
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-150 text-sm cursor-pointer ${isActive ? 'bg-pale text-sky' : 'text-gray-600 hover:bg-earth hover:text-night'
                        }`}
                    >
                      <span className={isActive ? 'text-sky' : 'text-gray-400'}>{section.icon}</span>
                      <span className="flex-1 text-left" style={{ fontWeight: isActive ? 500 : 400 }}>{section.label}</span>
                      {isRelatedClaims && (
                        <button
                          type="button"
                          onClick={(e) => {
                            e.stopPropagation();
                            setIsRelatedClaimsExpanded(!isRelatedClaimsExpanded);
                          }}
                      ...
                      {isActive && !isRelatedClaims && <div className="w-1.5 h-1.5 rounded-full bg-sky flex-shrink-0" />}
                    </div>
                    {isRelatedClaims && isRelatedClaimsExpanded && (
                      <div className="ml-7 pl-2 border-l border-pebble space-y-0.5 mt-1 mb-1">
                        {[
                          ...
                          const isSubActive = activeWorkspaceSection === 'Related Claims' && relatedClaimsSubFilter === sub.id;
                          return (
                            <button
                              key={sub.id}
                              onClick={() => {
                            ...
                            >
                              <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 transition-colors ${isSubActive ? 'bg-sky' : 'bg-gray-300'}`} />
                              <span className="text-left flex-1 truncate">{sub.label}</span>
                            </button>
                          );
                        })}
                      </div>
          ...
          <div className="px-4 py-3 border-t border-pebble">
            <div className="text-xs text-gray-400">
              {WORKSPACE_SECTIONS.findIndex(s => s.id === activeWorkspaceSection) + 1} / {WORKSPACE_SECTIONS.length} sections
            </div>
          </div>
        </div>
      </aside>
  ...
  if (activeModule === 'Products') {
    if (isInProductDetail) {
      const PRODUCT_DETAIL_SECTIONS: { label: ProductSection; icon: ReactElement; count?: number }[] = [
        { label: 'Product Details', icon: <Package className="w-4 h-4" /> },
        { label: 'Parent Products', icon: <GitBranch className="w-4 h-4" />, count: 1 },
        { label: 'Claims', icon: <FileText className="w-4 h-4" />, count: 7 },
        { label: 'Available Parent Claims', icon: <FileText className="w-4 h-4" />, count: 4 },
        { label: 'Available Child Claims', icon: <FileText className="w-4 h-4" />, count: 2 },
        { label: 'Child Products', icon: <Package className="w-4 h-4" />, count: 3 },
        { label: 'Related Assets', icon: <Paperclip className="w-4 h-4" />, count: 2 },
        { label: 'Related Projects', icon: <FolderOpen className="w-4 h-4" />, count: 2 },
      ];
      return (
        <aside className={sidebarClass}>
          <CollapseButton />
                  ...
                  return (
                    ...
                    >
                      ...
                      {section.count !== undefined && (
                        <span className={`px-1.5 py-0.5 rounded-full text-xs flex-shrink-0 ${isActive ? 'bg-sky/20 text-sky' : 'bg-earth text-gray-400'}`}>
                          {section.count}
                        </span>
                      )}
                      {isActive && <div className="w-1.5 h-1.5 rounded-full bg-sky flex-shrink-0" />}
        ...
        <div className={`flex flex-col h-full w-64 ${isCollapsed ? 'invisible opacity-0' : 'visible opacity-100'} transition-all duration-200`}>
          <div className="p-3 flex-1">
            <div className="mb-2">
              ...
              >
                <span>Products</span>
                {isProductsExpanded ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />}
              </button>
                ...
                <nav className="mt-1 space-y-0.5">
                  {PRODUCT_VIEWS.map(view => {
                    if (view.id === 'Saved Views') {
                      const isAnyProdSavedActive = activeProductListView.startsWith('Saved:');
                      return (
                        <div key={view.id} className="space-y-0.5">
                          <button
                            onClick={() => {
                              setIsProductSavedViewsFolderExpanded(!isProductSavedViewsFolderExpanded);
                            ...
                            className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-150 text-sm ${
                              isAnyProdSavedActive || activeProductListView === 'Saved Views' ? 'bg-pale text-sky' : 'text-gray-600 hover:bg-earth hover:text-night'
                            }`}
                          >
                            <span className={isAnyProdSavedActive || activeProductListView === 'Saved Views' ? 'text-sky' : 'text-gray-400'}>{view.icon}</span>
                            <span className="flex-1 text-left" style={{ fontWeight: isAnyProdSavedActive || activeProductListView === 'Saved Views' ? 500 : 400 }}>{view.label}</span>
                            {isProductSavedViewsFolderExpanded ? <ChevronDown className="w-3.5 h-3.5 text-gray-400" /> : <ChevronRight className="w-3.5 h-3.5 text-gray-400" />}
                          </button>
                          {isProductSavedViewsFolderExpanded && productSavedViews && productSavedViews.map(sv => {
                            const isSvActive = activeProductListView === `Saved: ${sv.name}`;
                            return (
                              <button
                                key={sv.id}
                                ...
                                className={`w-full flex items-center gap-2 pl-8 pr-3 py-1.5 rounded-lg transition-all duration-150 text-xs ${
                                  isSvActive ? 'bg-sky/5 text-sky font-medium' : 'text-gray-500 hover:bg-earth hover:text-night'
                                }`}
                              >
                                <span className="truncate flex-1 text-left">{sv.name}</span>
                                {isSvActive && <div className="w-1 h-1 rounded-full bg-sky flex-shrink-0" />}
                              </button>
                            );
                          })}
                        </div>
                    ...
                    const isActive = isHierarchy
                      ...
                      : activeProductListView === view.id && activeProductView !== 'hierarchy';
                    return (
                      <button
                        key={view.id}
                        onClick={() => {
                      ...
                      >
                        <span className={isActive ? 'text-sky' : 'text-gray-400'}>{view.icon}</span>
                        <span style={{ fontWeight: isActive ? 500 : 400 }}>{view.label}</span>
  ...
  if (activeModule === 'Claims') {
    if (isInClaimsWorkspace) {
      const CLAIMS_WORKSPACE_SECTIONS = [
        ...
        { label: 'Support Strategy & Substantiation', icon: <Shield className="w-4 h-4" /> },
        { label: 'Final Risk Summary', icon: <Shield className="w-4 h-4" /> },
        { label: 'Risk Level Assessments', icon: <Shield className="w-4 h-4" /> },
            ...
            <nav className="p-3 flex-1">
              <div className="space-y-0.5">
                {CLAIMS_WORKSPACE_SECTIONS.map(section => {
                  const isActive = activeClaimsWorkspaceSection === section.label;
                  return (
                    <button
                      key={section.label}
                      onClick={() => onClaimsWorkspaceSectionChange?.(section.label)}
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-150 text-sm ${isActive ? 'bg-pale text-sky' : 'text-gray-600 hover:bg-earth hover:text-night'
                        }`}
                    >
                      <span className={isActive ? 'text-sky' : 'text-gray-400'}>{section.icon}</span>
    ...
    const CLAIM_BASE_VIEWS: ClaimBaseView[] = ['Global Claims', 'Regional Claims', 'Local Claims', 'Local Claims SKU'];
    const CLAIM_WORK_VIEWS: ClaimWorkView[] = ['Support Strategy & Substantiation', 'Risk Level Assessments', 'Final Risk Summary'];
      ...
      <aside className={sidebarClass}>
        ...
        <div className={`flex flex-col h-full w-64 ${isCollapsed ? 'invisible opacity-0' : 'visible opacity-100'} transition-all duration-200`}>
          <div className="p-3 flex-1 overflow-y-auto">
            <div className="px-3 py-2 text-xs text-gray-400 uppercase tracking-wider" style={{ fontWeight: 500 }}>Base Views</div>
                ...
                <div key={view} className="mb-1">
                  <button
                    onClick={() => {
                      ...
                      setExpandedClaimSections(prev => ({ ...prev, [view]: !prev[view] }));
                    }}
                    className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-150 text-sm ${isActive ? 'bg-pale text-sky' : 'text-gray-600 hover:bg-earth hover:text-night'
                      }`}
                  >
                    {isExpanded ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />}
                    <FileText className="w-4 h-4" />
                    <span className="flex-1 text-left" style={{ fontWeight: isActive ? 500 : 400 }}>{view}</span>
                    {isActive && <div className="w-1.5 h-1.5 rounded-full bg-sky" />}
                  </button>
                  {isExpanded && (() => {
                    const claimType = CLAIM_TYPE_MAP[view];
                    const typeFilteredClaims = claims.filter(c => c.claimType === claimType);
                    const recentClaims = typeFilteredClaims.slice(-3).reverse();
                    const favClaims = typeFilteredClaims.filter(c => c.isFavorite);
                    ...
                    return (
                      <div className="ml-9 mt-1 space-y-0.5">
                        {/* Recents toggle */}
                        <button
                          onClick={() => setClaimSubOpen(p => ({ ...p, [view]: p[view] === 'recents' ? null : 'recents' }))}
                        ...
                        >
                          ...
                          {sub === 'recents' ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                        </button>
                        {sub === 'recents' && (
                          <div className="ml-5 space-y-0.5">
                            {recentClaims.length === 0 ? (
                              <p className="px-3 py-1.5 text-xs text-gray-400 italic">No recent claims</p>
                            ) : recentClaims.map(c => (
                              <button key={c.id} onClick={() => onClaimClick?.(c)}
                              ...
                              >
                                {getPrimaryStatement(c)?.slice(0, 40) || c.id}…
                              </button>
                            ))}
                          </div>
                        )}
                        {/* Favorites toggle */}
                        <button
                          onClick={() => setClaimSubOpen(p => ({ ...p, [view]: p[view] === 'favorites' ? null : 'favorites' }))}
                          className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-earth hover:text-night transition-all duration-150"
                        >
                          <Star className="w-3.5 h-3.5 text-gray-400" />
                          <span className="flex-1 text-left">Favorites</span>
                          {sub === 'favorites' ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                        </button>
                        {sub === 'favorites' && (
                          <div className="ml-5 space-y-0.5">
                            {favClaims.length === 0 ? (
                              <p className="px-3 py-1.5 text-xs text-gray-400 italic">No favorites yet</p>
                            ) : favClaims.map(c => (
                              <button key={c.id} onClick={() => onClaimClick?.(c)}
                                className="w-full text-left px-3 py-1.5 text-xs text-gray-600 hover:bg-earth hover:text-night rounded-lg transition-colors truncate"
                                title={getPrimaryStatement(c)}
                              >
                                ...
                                {getPrimaryStatement(c)?.slice(0, 36) || c.id}…
                              </button>
                            ))}
                          </div>
                        )}
            ...
            {/* Work Views */}
            <div className="px-3 py-2 text-xs text-gray-400 uppercase tracking-wider" style={{ fontWeight: 500 }}>Views</div>
            {CLAIM_WORK_VIEWS.map(view => {
              const isActive = activeClaimsWorkView === view;
              const iconMap: Record<ClaimWorkView, ReactElement> = {
                'Support Strategy & Substantiation': <FileText className="w-4 h-4" />,
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all duration-150 ${isActive ? 'bg-pale text-sky' : 'text-gray-600 hover:bg-earth hover:text-night'
                    }`}
                >
                  <span className={isActive ? 'text-sky' : 'text-gray-400'}>{iconMap[view]}</span>
                  <span className="flex-1 text-left" style={{ fontWeight: isActive ? 500 : 400 }}>{view}</span>
                  {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-sky" />}
                </button>
              );
            ...
            {isProjectsExpanded && (
              <nav className="mt-1 space-y-0.5">
                {PROJECT_VIEWS.map(view => {
                  if (view.id === 'Saved Views') {
                    const isAnySavedViewActive = activeView.startsWith('Saved View:');
                    return (
                      <div key={view.id} className="space-y-0.5">
                        <button
                          onClick={() => {
                            setIsSavedViewsFolderExpanded(!isSavedViewsFolderExpanded);
                            // onViewChange('Saved Views');
                          }}
                          className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-150 text-sm ${
                            isAnySavedViewActive || activeView === 'Saved Views' ? 'bg-pale text-sky' : 'text-gray-600 hover:bg-earth hover:text-night'
                          }`}
                        >
                          <span className={isAnySavedViewActive || activeView === 'Saved Views' ? 'text-sky' : 'text-gray-400'}>{view.icon}</span>
                          <span className="flex-1 text-left" style={{ fontWeight: isAnySavedViewActive || activeView === 'Saved Views' ? 500 : 400 }}>{view.label}</span>
                          {isSavedViewsFolderExpanded ? <ChevronDown className="w-3.5 h-3.5 text-gray-400" /> : <ChevronRight className="w-3.5 h-3.5 text-gray-400" />}
                        </button>
                        {isSavedViewsFolderExpanded && projectSavedViews && projectSavedViews.map(sv => {
                          const isSvActive = activeView === `Saved View: ${sv.name}`;
                          return (
                            <button
                              key={sv.id}
